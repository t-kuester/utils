...
def determine_include(directory):
	...
	"""
	backup, change = directory.last_backup, directory.last_change
	directory.include = not (backup and change) or backup < change
...
def perform_backup(config):
	...
	for directory in config.directories:
		backup_directory(directory, config.name_pattern, config.target_dir)
...
def backup_directory(directory, name_pattern, target_dir):
	...
	date = "TODAY" # TODO
	filename = namepattern.format(pareent=parent, dirname=dirname, date=date)
	# create archive file
...
def all_files(path):
	...
	"""
	return (os.path.join(d, f) for d, _, fs in os.walk(to_compress) for f in fs)
...
def main():
	...
	"""TODO
	- command line argument: config file, auto-include
	- load config file
	- determine last change (always)
	- determine auto-include (optional)
	- perform backup
	- show some message?
	- save config file
	"""
